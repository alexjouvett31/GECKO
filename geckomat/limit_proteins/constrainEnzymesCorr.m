...
function [model,sol,MW_out,counter_out] = constrainEnzymesCorr(model,Ptot,sigma,f,pIDs,data,MW,counter)
...
model.ub(loc(j)) = 1.25*mass_fracs(j)*total_protein_mass*sigma./model.MWs(loc2);
model.lb(loc(j)) = 0.75*mass_fracs(j)*total_protein_mass*sigma./model.MWs(loc2);
